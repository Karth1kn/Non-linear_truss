...
def arc_length_eqn(increment, iteration, it_max, gsM, Residual, Fext, Du, Dl, ds):
    ...
    dU__ = slin.solve(gsM , bc(Residual, bc_lst)) #scipy.sparse.linalg.splu(gsM).solve(bc(Residual, bc_lst))    #
    dU_ = slin.solve(gsM , Fext) #scipy.sparse.linalg.splu(gsM).solve(Fext)     #
    dU__ = -1*dU__
